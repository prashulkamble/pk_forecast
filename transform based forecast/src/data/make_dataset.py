...
def read_dataset(private_sub=False):
    ...
    path  = 'D:\python_code\M5 Competition\A5_solution\walmart_M5_forecasting\source_data\\'
    try:
        ...
        calendar = pd.read_csv(
            path + "calendar.csv", parse_dates=["date"]
        )
        sales_train = pd.read_csv(
            path + f"sales_train_{sales_suffix}.csv"
        )
    except FileNotFoundError:
        sell_prices = pd.read_csv(path + "sell_prices.csv")
        sample_submission = pd.read_csv(
            path + "sample_submission.csv", index_col="id",
        )
        calendar = pd.read_csv(
            path + "calendar.csv", parse_dates=["date"]
        )
